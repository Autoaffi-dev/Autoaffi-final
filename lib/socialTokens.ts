...
function isExpiringSoon(
  ...
  if (!expiresAtIso) {
     ...
     * Exempel:
     *
     * Facebook System User-token med Never Expire
     * kan sakna token_expires_at.
     *
     * Då ska vi inte försöka refresha den.
     */
    return false;
  }
...
/*
 * Hindrar att två samtidiga requests försöker
 * refresha samma token samtidigt i samma serverinstans.
 */
const inflight =
  new Map<
    string,
...
async function updateRowTokens(
  args: {
    rowId: string;
    accessToken: string;
...
/*
 * Instagram API with Instagram Login har ett
 * separat token-refresh-flöde.
 *
 ...
 * Ingen Facebook Graph token exchange.
 */
async function refreshInstagramAccessToken(
  ...
  if (
    ...
    console.error(
      "[social-tokens] Instagram refresh failed",
      {
        status:
          response.status,
    ...
    throw new TokenRefreshError(
      {
        ...
        message:
          `instagram_refresh_failed:${reason}`,
...
async function refreshTikTokAccessToken(
  ...
  return {
     ...
     * Senast returnerade token ersätter den gamla.
     */
    refreshToken:
      body.refresh_token,
...
export async function getValidAccessToken(
  ...
  const refreshPromise =
    (async () => {
      ...
      /*
       * Tokenen används direkt om den inte
      ...
      try {
            ...
            await updateRowTokens(
              {
                ...
                /*
                 * Instagram Login använder inte separat
                 * refresh_token i detta flöde.
                 */
                refreshToken:
                  undefined,
                ...
                metaPatch: {
                  ...
                  token_refresh_note:
                    "instagram_long_lived_token_refreshed",
        ...
        /*
         * LinkedIn hanteras först när vi vet om
         * den faktiska LinkedIn-appen får
         * programmatisk refresh-token.
         *
         * Om ingen refresh finns krävs reconnect.
         */
        throw new Error(
 ...
 */
export async function markLastSynced(
  args: {
    userId: string;
    platform:
      | SocialPlatform
